...
function createStreamInfoPanel(controlBar, playInfo, monitor) {
    ...
    statsDetails.style.gap = '8px';    // 填充流信息
    function updateStreamInfo() {
        ...
        if (!video || !audio) {
            console.warn('[LitePlayer] 無法更新流信息，因為 video 或 audio 元素未找到');
            return;
        }
        ...
        try {
            ...
            if (videoBuffered.length > 0) {
                ...
                for (let i = 0; i < videoBuffered.length; i++) {
                    if (video.currentTime >= videoBuffered.start(i) && video.currentTime <= videoBuffered.end(i)) {
                        videoBufferedTime = videoBuffered.end(i) - video.currentTime;
                        break;
                    }
                }
                // 如果沒有找到包含當前時間的區間，使用最後一個區間
                if (videoBufferedTime === 0 && videoBuffered.length > 0) {
                    const lastIndex = videoBuffered.length - 1;
                    if (video.currentTime <= videoBuffered.end(lastIndex)) {
                        videoBufferedTime = videoBuffered.end(lastIndex) - video.currentTime;
                    }
                }
            }
        } catch (e) {
            console.warn('[LitePlayer] 計算視頻緩存時間失敗:', e);
        }

        // 計算音頻緩存時間
        let audioBufferedTime = 0;
        try {
            const audioBuffered = audio.buffered;
            if (audioBuffered.length > 0) {
                // 找到包含當前播放時間的緩衝區間
                for (let i = 0; i < audioBuffered.length; i++) {
                    if (audio.currentTime >= audioBuffered.start(i) && audio.currentTime <= audioBuffered.end(i)) {
                        audioBufferedTime = audioBuffered.end(i) - audio.currentTime;
                        break;
                    }
                }
                // 如果沒有找到包含當前時間的區間，使用最後一個區間
                if (audioBufferedTime === 0 && audioBuffered.length > 0) {
                    const lastIndex = audioBuffered.length - 1;
                    if (audio.currentTime <= audioBuffered.end(lastIndex)) {
                        audioBufferedTime = audioBuffered.end(lastIndex) - audio.currentTime;
                    }
                }
            }
        } catch (e) {
            console.warn('[LitePlayer] 計算音頻緩存時間失敗:', e);
        }
    ...
    // 添加到控制欄
    controlBar.appendChild(infoPanel);    // 初始更新信息
    updateStreamInfo();
    ...
    // 清理定時器的引用
    if (!playerEventHandlers.syncHandlers) {
        playerEventHandlers.syncHandlers = [];
    }
    playerEventHandlers.syncHandlers.push(() => clearInterval(updateInterval));
...
function createPreloadControlPanel(controlBar, preloader) {
    ...
    const presets = [
        { name: '省電模式', concurrent: 2, segment: 512 * 1024, timeout: 20, retry: 2 },
        { name: '標準模式', concurrent: 4, segment: 2 * 1024 * 1024, timeout: 30, retry: 3 },
        { name: '高速模式', concurrent: 8, segment: 4 * 1024 * 1024, timeout: 15, retry: 1 },
        { name: '極速模式', concurrent: 12, segment: 8 * 1024 * 1024, timeout: 10, retry: 1 }
    ];
    ...
    presets.forEach(preset => {
        ...
        button.addEventListener('click', () => {
            ...
            segmentSizeSelect.value = preset.segment;
            ...
            preloader.setConfig({
                ...
                videoSegmentSize: preset.segment,
                audioSegmentSize: preset.segment
            });
